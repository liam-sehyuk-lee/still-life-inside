...
##
## 두 개의 '#' 표시로 시작되는 줄은 주석이며, 그것을 없애지 말아야 합니다. 한
## 개의 '#' 표시로 시작되는 줄은 주석 처리된 코드로 필요한 경우 제거해도 됩니다.


## 기본 ##########################################################################

## 인간이 읽을 수 있는 게임의 이름. 기본 윈도우의 제목으로 사용되며, 인터페이
## 스와 오류 보고에서 보여집니다.
##
## 문자열을 _()로 둘러 쌓으면 씌우면 번역의 대상으로 표시됩니다.

define config.name = _("Still Life Inside")
...
## 게임의 about 스크린에 배치되는 텍스트입니다. 텍스트를 삼중 따옴표 사이에 배
## 치하고 단락 사이에 빈 줄을 남겨 둡니다.
...
define build.name = "StillLifeInside"


## 음악과 음향 ######################################################################

## 이 세 가지 변수는 무엇보다도 플레이어에게 기본적으로 표시되는 믹서를 제어합
## 니다. 이 중 하나를 False로 설정하면 해당 믹서가 숨겨집니다.
...
## 창 관리 ########################################################################
##
## 이것은 대사 창이 표시됐을 때 제어합니다. 만약 "show"면, 그것은 상항 표시됩
## 니다. 만약 "hide"면, 그것은 대사가 주어질 때만 표시됩니다. 만약 "auto"면, 창은
...
## 기본 auto-forward 지연 시간입니다. 숫자가 클수록 대기 시간이 길어지며, 0 ~
## 30이 유효한 범위가 됩니다.
...
##
## 이것은 일반적으로 변경해서는 안 되며, 항상 표현형식이 아닌 정확한 문자열이
## 어야 합니다.

define config.save_directory = "StillLifeInside-1754627808"
...
init python:
    ...
    ## 다음 함수는 파일 패턴을 사용합니다. 파일 패턴은 대/소문자를 구분하지 않
    ## 으며, /의 유무와 관계없이 기본 디렉터리의 상대 경로와 일치합니다. 여러 패
    ## 턴이 일치하면 첫 번째 패턴이 사용됩니다.
    ##
    ## 패턴 있음:
    ##
    ## / 는 디렉토리 구분 기호입니다.
    ...
    ## 파일들의 매칭 문서 패턴은 맥앱(Mac App) 빌드에서 중복되므로 app 및 zip
    ## 파일에 모두 나타납니다.
