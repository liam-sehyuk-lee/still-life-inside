...
## gui.init의 호출은 스타일을 합리적인 기본값으로 재설정하고, 게임의 너비
## (width)와 높이(height)를 설정합니다.
init python:
    gui.init(1920, 1080)
...
## 텍스트 버튼(text button)이 선택(selected)됐거나 커서를 올리지(hovered) 않았
## 을 때 사용됩니다.
...
## 텍스트 버튼(text button)에 선택됐지만(selected) 포커스되지(focused) 않았을
## 때 사용됩니다. 버튼(button)은 현재 화면이거나 설정값인 경우 선택됨(selected)
## 이 됩니다.
...
## 화면에 텍스트박스를 세로로 배치합니다. 0.0은 최상단, 0.5는 중앙, 그리고 1.0
## 은 최하단입니다.
...
## 캐릭터들의 이름이 들어 있는 박스의 너비, 높이, 그리고 테두리입니다. 혹은 그
## 것을 None으로 자동 설정할 수 있습니다.
define gui.namebox_width = None
define gui.namebox_height = None

## 캐릭터의 이름이 들어 있는 박스의 테두리를 좌측, 상단, 우측, 하단의 순서로
## 정합니다.
...
## 픽셀값에서 버튼의 너비와 높이입니다. 만약 None이면, 렌파이가 크기를 계산합
## 니다.
...
## 만약 참(True)이면, 배경 이미지는 바둑판식으로 배열(tiled)될 것입니다. 만약
## 거짓(False)이면, 배경 이미지는 선으로 채워질(scaled) 것입니다.
...
## 당신은 또한 설정된 이름의 변수를 추가함으로써 당신만의 커스텀을 추가할 수
## 있습니다. 예를 들어, 다음 행의 주석 표시를 제거하여 탐색(navigation) 버튼의
## 너비를 설정할 수 있습니다.
...
## 파일 슬롯 버튼 ####################################################################
##
## 파일 슬롯 버튼은 버튼의 특별한 종류입니다. 그것은 썸네일 이미지나 저장 슬롯
## 의 콘텐츠를 설명하는 글자를 포함합니다. GUI/버튼에서 저장 슬롯은 버튼의 다른
## 종류와 같은 이미지 파일을 사용합니다.
...
## 막대, 스크롤바, 슬라이더 ##############################################################
##
## 이러한 설정은 막대와 스크롤바, 그리고 슬라이더의 보여지는 것과 크기를 제어
## 합니다.
##
## 기본 GUI는 슬라이더와 세로 스크롤 막대만 사용합니다. 다른 모든 막대는 제작
## 자가 작성한 화면에서만 사용됩니다.
...
## 대사록 화면 항목의 높이를 지정하거나 None으로 하여 높이를 성능에 맡길 수 있
## 습니다.
...
## NVL-모드 항목의 높이입니다. 이것을 None으로 설정하면 항목들은 동적으로 높이
## 를 조정합니다.
define gui.nvl_height = 173

## gui.nvl_height 값이 None일 때 NVL-모드 항목들, 그리고 NVL-모드 항목들과
## NVL-모드 메뉴간의 간의 간격입니다.
...
## nvl_thought 글자의 위치, 너비, 정렬(nvl_narrator 캐릭터에 의해 표시되는 글
## 자)입니다.
...
init python:
    ...
    ## 이것은 휴대전화에서 다양한 GUI 요소들의 크기와 간격을 쉽게 보일 수 있도
    ## 록 변경합니다.
    @gui.variant
    def small():
